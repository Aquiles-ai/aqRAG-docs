  ...
  for (let i = 0; i < sections.length; i++) {
    ...
    if (/^##\s+/.test(part) || /^###\s+/.test(part)) {
      ...
      if (
        ...
        !searchIndex.some((item) => item.filename === filename && item.title === documentTitle)
      ) {
        searchIndex.push({
          filename: filename,
          title: documentTitle,
...
function renderResults(results, query) {
  ...
  results.slice(0, 10).forEach((match) => {
    ...
    resultElement.className =
      "p-3 rounded-lg hover:bg-gray-50 cursor-pointer border border-transparent hover:border-gray-100 mb-2"
    ...
    if (matchIndex !== -1) {
      ...
      contextSnippet =
        (start > 0 ? "..." : "") + match.context.substring(start, end) + (end < match.context.length ? "..." : "")
    } else {
      contextSnippet = match.context.substring(0, 150) + (match.context.length > 150 ? "..." : "")
    }

    const regex = new RegExp(escapeRegExp(query), "gi")
    const highlightedContext = escapeHtml(contextSnippet).replace(
      regex,
      (m) => `<span class="px-1 rounded bg-yellow-100 text-yellow-800">${m}</span>`,
    )
